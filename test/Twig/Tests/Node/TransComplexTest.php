...
class Twig_Tests_Node_TransComplexTest extends Twig_Test_NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(gettext("J\'ai %user_pommes%"), array("%user_pommes%" => $this->getAttribute('.$this->getVariableGetter('user').', "pommes", array()), ));');

        // Testing somewhat more complex "complex" variables.
        $node = $this->parseTrans('{% trans %}J\'ai {{user["first"].pommes}}{% endtrans %}');
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(gettext("J\'ai %user_first_pommes%"), array("%user_first_pommes%" => $this->getAttribute($this->getAttribute('.$this->getVariableGetter('user').', "first", array(), "array"), "pommes", array()), ));');

        // Testing multiple complex fields gets same name.
        $node = $this->parseTrans('{% trans %}Je suis {{user.name}} {{user.name}}{% endtrans %}');
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(gettext("Je suis %user_name% %user_name%"), array("%user_name%" => $this->getAttribute('.$this->getVariableGetter('user').', "name", array()), ));');
        ...
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(gettext("Je suis %user_name% %user_name_2%"), array("%user_name%" => twig_upper_filter($this->env, $this->getAttribute('.$this->getVariableGetter('user').', "name", array())), "%user_name_2%" => twig_lower_filter($this->env, $this->getAttribute('.$this->getVariableGetter('user').', "name", array())), ));');

        // Testing plurals still works using complex algorithm.
        $node = $this->parseTrans('{% trans %}I have {{count}} apple{% plural 12 %}I have {{count}} apples{% endtrans %}');
        ...
        $node = $this->parseTrans('{% trans %}Hi {{user.name}}, i have {{count}} apple{% plural 12 %}Hi {{user.name}} I have {{count}} apples{% endtrans %}');
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(ngettext("Hi %user_name%, i have %count% apple", "Hi %user_name% I have %count% apples", abs(12)), array("%user_name%" => $this->getAttribute('.$this->getVariableGetter('user').', "name", array()), "%count%" => abs(12), ));');
        ...
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(ngettext("Sorry %user_name%, i only have one apple", "YUP! %user_name_2% I HAVE %count% APPLES", abs(12)), array("%user_name%" => $this->getAttribute('.$this->getVariableGetter('user').', "name", array()), "%user_name_2%" => twig_upper_filter($this->env, $this->getAttribute('.$this->getVariableGetter('user').', "name", array())), "%count%" => abs(12), ));');
        ...
        $tests[] = array($node, self::DEBUG_LINE.'echo strtr(ngettext("Sorry %name%, i only have one apple", "YUP! %name_2% I HAVE %count% APPLES", abs(12)), array("%name%" => $this->getAttribute('.$this->getVariableGetter('user').', "name", array()), "%name_2%" => twig_upper_filter($this->env, $this->getAttribute('.$this->getVariableGetter('user').', "name", array())), "%count%" => abs(12), ));');
    ...
    private function parseTrans($source)
    {
        ...
        $module = $parser->parse($lexer->tokenize($source));
