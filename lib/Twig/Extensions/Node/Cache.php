...
class Twig_Extensions_Node_Cache extends Twig_Node
{
    ...
    public function compile(Twig_Compiler $compiler)
    {
        ...
        $compiler
            ->addDebugInfo($this)
            ->write("if (\$this->env->getExtension('cache')->cache_exists(")
            ->subcompile($cache_key)
            ->write(")) {\n")
                ->indent()
                ->write("echo \$this->env->getExtension('cache')->cache_get(")
                ->subcompile($cache_key)
                ->write(");\n")
                ->outdent()
            ->write("} else {\n")
                ...
                ->write("\$this->env->getExtension('cache')->cache_set(")
                ->subcompile($cache_key)
                ->write(", \$body);\n")
                ->write("echo \$body;\n")
                ->outdent()
