 ...
 *
 * Sluggize lifted from Doctrine 1 inflector
 *
...
class Twig_Extensions_Extension_Text extends Twig_Extension
{
    ...
    public function getFilters()
    {
        return array(
            ...
            'sluggize' => new Twig_Filter_Function('twig_sluggize_filter', array('needs_environment' => true)),
        );
    }
...
if (function_exists('mb_get_info')) {
    ...
    function twig_sluggize_filter(Twig_Environment $env, $string) {
               // Remove all non url friendly characters with the unaccent function
        $text = unaccentHelper($string);
  ...
  function twig_wordwrap_filter(Twig_Environment $env, $value, $length = 80, $separator = "\n", $preserve = false)
  {
     ...
     *
     * Modified source of original provided by bmorel [at] ssi [dot] fr
     *
     * @param string $text Text to be inspected
     * @return boolean
     */
    function seemsUtf8Helper($text)
    {
        for ($i = 0, $l = strlen($text); $i < $l; $i++) {
            if (ord($text[$i]) < 0x80) continue; // 0bbbbbbb
            else if ((ord($text[$i]) & 0xE0) == 0xC0) $n = 1; // 110bbbbb
            else if ((ord($text[$i]) & 0xF0) == 0xE0) $n = 2; // 1110bbbb
            else if ((ord($text[$i]) & 0xF8) == 0xF0) $n = 3; // 11110bbb
            else if ((ord($text[$i]) & 0xFC) == 0xF8) $n = 4; // 111110bb
            else if ((ord($text[$i]) & 0xFE) == 0xFC) $n = 5; // 1111110b
            else return false; // Does not match any model

            for ($j = 0; $j < $n; $j++) { // bytes matching 10bbbbbb follow?
                if ((++$i == $l) || ((ord($text[$i]) & 0xC0) != 0x80)) {
                    return false;
                }
            }
        }
